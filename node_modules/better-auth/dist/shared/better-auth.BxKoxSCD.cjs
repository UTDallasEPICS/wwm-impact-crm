...
const error$1 = require('@better-auth/core/error');
...
const url = require('./better-auth.DBTqCDAW.cjs');
...
const originCheckMiddleware = middleware.createAuthMiddleware(async (ctx) => {
  ...
  if (isSimpleRequest(headers) && !ctx.context.skipCSRFCheck) {
    throw new betterCall.APIError("FORBIDDEN", { message: "Invalid request" });
  }
...
`,
  ...
  if (useCookies && !ctx.context.skipCSRFCheck && !ctx.context.skipOriginCheck) {
    if (!originHeader || originHeader === "null") {
      throw new betterCall.APIError("FORBIDDEN", { message: "Missing or null Origin" });
    }
...
const originCheck = (getValue) => middleware.createAuthMiddleware(async (ctx) => {
  if (!ctx.request) {
    return;
  }
  const { context } = ctx;
...
const sendVerificationEmail = middleware.createAuthEndpoint(
...
const verifyEmail = middleware.createAuthEndpoint(
  "/verify-email",
  {
    method: "GET",
  ...
  if (dbUser) {
    ...
    if (!hasBeenLinked) {
      ...
      try {
        ...
        env.logger.error("Unable to link account", e);
        return {
...
const signInSocial = middleware.createAuthEndpoint(
  "/sign-in/social",
  {
    method: "POST",
    body: z__namespace.object({
       ...
       */
      provider: socialProviders.SocialProviderListEnum,
      /**
  ...
  async (c) => {
    ...
    if (!provider) {
      ...
      throw new betterCall.APIError("NOT_FOUND", {
        message: error$1.BASE_ERROR_CODES.PROVIDER_NOT_FOUND
      });
    }
    if (c.body.idToken) {
      if (!provider.verifyIdToken) {
        ...
        throw new betterCall.APIError("NOT_FOUND", {
          message: error$1.BASE_ERROR_CODES.ID_TOKEN_NOT_SUPPORTED
        });
      }
      ...
      if (!valid) {
        ...
        throw new betterCall.APIError("UNAUTHORIZED", {
          message: error$1.BASE_ERROR_CODES.INVALID_TOKEN
        });
      }
      ...
      if (!userInfo || !userInfo?.user) {
        ...
        throw new betterCall.APIError("UNAUTHORIZED", {
          message: error$1.BASE_ERROR_CODES.FAILED_TO_GET_USER_INFO
        });
      }
      if (!userInfo.user.email) {
        ...
        throw new betterCall.APIError("UNAUTHORIZED", {
          message: error$1.BASE_ERROR_CODES.USER_EMAIL_NOT_FOUND
        });
      }
...
const signInEmail = middleware.createAuthEndpoint(
  "/sign-in/email",
  {
    method: "POST",
    body: z__namespace.object({
  ...
  async (ctx) => {
    ...
    if (!isValidEmail.success) {
      throw new betterCall.APIError("BAD_REQUEST", {
        message: error$1.BASE_ERROR_CODES.INVALID_EMAIL
      });
    }
    const user = await ctx.context.internalAdapter.findUserByEmail(email, {
    ...
    if (!user) {
      ...
      throw new betterCall.APIError("UNAUTHORIZED", {
        message: error$1.BASE_ERROR_CODES.INVALID_EMAIL_OR_PASSWORD
      });
    }
    ...
    if (ctx.context.options?.emailAndPassword?.requireEmailVerification && !user.user.emailVerified) {
      if (!ctx.context.options?.emailVerification?.sendVerificationEmail) {
        throw new betterCall.APIError("FORBIDDEN", {
          message: error$1.BASE_ERROR_CODES.EMAIL_NOT_VERIFIED
        });
      }
    ...
    if (!session) {
      ...
      throw new betterCall.APIError("UNAUTHORIZED", {
        message: error$1.BASE_ERROR_CODES.FAILED_TO_CREATE_SESSION
      });
    }
...
const callbackOAuth = middleware.createAuthEndpoint(
  "/callback/:id",
  {
    method: ["GET", "POST"],
...
const signOut = middleware.createAuthEndpoint(
  "/sign-out",
  {
    method: "POST",
    requireHeaders: true,
  ...
  async (ctx) => {
    ...
    if (!sessionCookieToken) {
      ...
      throw new betterCall.APIError("BAD_REQUEST", {
        message: error$1.BASE_ERROR_CODES.FAILED_TO_GET_SESSION
      });
    }
...
const requestPasswordReset = middleware.createAuthEndpoint(
...
const forgetPassword = middleware.createAuthEndpoint(
  "/forget-password",
  {
    method: "POST",
    body: z__namespace.object({
...
const requestPasswordResetCallback = middleware.createAuthEndpoint(
...
const resetPassword = middleware.createAuthEndpoint(
  "/reset-password",
  {
    method: "POST",
    query: z__namespace.object({
  ...
  async (ctx) => {
    ...
    if (newPassword.length < minLength) {
      throw new betterCall.APIError("BAD_REQUEST", {
        message: error$1.BASE_ERROR_CODES.PASSWORD_TOO_SHORT
      });
    }
    if (newPassword.length > maxLength) {
      throw new betterCall.APIError("BAD_REQUEST", {
        message: error$1.BASE_ERROR_CODES.PASSWORD_TOO_LONG
      });
    }
...
const updateUser = () => middleware.createAuthEndpoint(
  "/update-user",
  {
    method: "POST",
  ...
  async (ctx) => {
    ...
    if (body.email) {
      throw new betterCall.APIError("BAD_REQUEST", {
        message: error$1.BASE_ERROR_CODES.EMAIL_CAN_NOT_BE_UPDATED
      });
    }
...
const changePassword = middleware.createAuthEndpoint(
  "/change-password",
  {
    method: "POST",
    body: z__namespace.object({
  ...
  async (ctx) => {
    ...
    if (!account || !account.password) {
      throw new betterCall.APIError("BAD_REQUEST", {
        message: error$1.BASE_ERROR_CODES.CREDENTIAL_ACCOUNT_NOT_FOUND
      });
    }
    ...
    if (!verify) {
      throw new betterCall.APIError("BAD_REQUEST", {
        message: error$1.BASE_ERROR_CODES.INVALID_PASSWORD
      });
    }
...
const setPassword = middleware.createAuthEndpoint(
  "/set-password",
  {
    method: "POST",
    body: z__namespace.object({
...
const deleteUser = middleware.createAuthEndpoint(
  "/delete-user",
  {
    method: "POST",
    use: [session.sensitiveSessionMiddleware],
  ...
  async (ctx) => {
    ...
    if (!ctx.body.password && ctx.context.sessionConfig.freshAge !== 0) {
      ...
      if (now - currentAge > freshAge * 1e3) {
        throw new betterCall.APIError("BAD_REQUEST", {
          message: error$1.BASE_ERROR_CODES.SESSION_EXPIRED
        });
      }
    }
...
const deleteUserCallback = middleware.createAuthEndpoint(
...
const changeEmail = middleware.createAuthEndpoint(
  "/change-email",
  {
    method: "POST",
    body: z__namespace.object({
    ...
    if (ctx.context.session.user.emailVerified !== true) {
      ...
      if (existing) {
        throw new betterCall.APIError("UNPROCESSABLE_ENTITY", {
          message: error$1.BASE_ERROR_CODES.USER_ALREADY_EXISTS_USE_ANOTHER_EMAIL
        });
      }
...
const error = middleware.createAuthEndpoint(
  "/error",
  {
    method: "GET",
    metadata: {
...
const ok = middleware.createAuthEndpoint(
  "/ok",
  {
    method: "GET",
    metadata: {
...
const listUserAccounts = middleware.createAuthEndpoint(
  "/list-accounts",
  {
    method: "GET",
    use: [session.sessionMiddleware],
...
const linkSocialAccount = middleware.createAuthEndpoint(
  "/link-social",
  {
    method: "POST",
    requireHeaders: true,
...
const unlinkAccount = middleware.createAuthEndpoint(
  "/unlink-account",
  {
    method: "POST",
    body: z__namespace.object({
  ...
  async (ctx) => {
    ...
    if (accounts.length === 1 && !ctx.context.options.account?.accountLinking?.allowUnlinkingAll) {
      throw new betterCall.APIError("BAD_REQUEST", {
        message: error$1.BASE_ERROR_CODES.FAILED_TO_UNLINK_LAST_ACCOUNT
      });
    }
    ...
    if (!accountExist) {
      throw new betterCall.APIError("BAD_REQUEST", {
        message: error$1.BASE_ERROR_CODES.ACCOUNT_NOT_FOUND
      });
    }
...
const getAccessToken = middleware.createAuthEndpoint(
  "/get-access-token",
  {
    method: "POST",
    body: z__namespace.object({
...
const refreshToken = middleware.createAuthEndpoint(
  "/refresh-token",
  {
    method: "POST",
    body: z__namespace.object({
...
const accountInfo = middleware.createAuthEndpoint(
  "/account-info",
  {
    method: "POST",
    use: [session.sessionMiddleware],
...
function toAuthEndpoints(endpoints, ctx) {
  ...
  for (const [key, endpoint] of Object.entries(endpoints)) {
    api[key] = async (context) => {
      ...
      if (result.response instanceof betterCall.APIError && env.shouldPublishLog(authContext.logger.level, "debug")) {
  ...
  for (const hook of hooks) {
    if (hook.matcher(context)) {
      const result = await hook.handler({
        ...
        if (e instanceof betterCall.APIError && env.shouldPublishLog(context.context.logger.level, "debug")) {
  ...
  for (const hook of hooks) {
    if (hook.matcher(context)) {
      const result = await hook.handler(context).catch((e) => {
        if (e instanceof betterCall.APIError) {
          if (env.shouldPublishLog(context.context.logger.level, "debug")) {
            e.stack = e.errorStack;
          }
          return {
            response: e,
...
exports.isSimpleRequest = isSimpleRequest;
