...
require('kysely');
require('./better-auth.B4q1j93R.cjs');
require('./better-auth.kVDH3hoR.cjs');
require('@better-auth/utils/otp');
require('./better-auth.Bx5mWHak.cjs');
require('./better-auth.Zk899y_h.cjs');
require('./better-auth.DtX8_eyE.cjs');
require('@better-fetch/fetch');
require('@better-auth/core/oauth2');
require('@better-auth/utils');
require('../plugins/custom-session/index.cjs');
require('./better-auth.BM1EYiYB.cjs');
const date = require('./better-auth.C1hdVENX.cjs');
const schema = require('./better-auth.DhDRLI4w.cjs');
...
function createApiKey({
  ...
  return middleware.createAuthEndpoint(
    ...
    async (ctx) => {
      ...
      const user = authRequired && !session$1 ? null : session$1?.user || { id: ctx.body.userId };
      if (!user?.id) {
        throw new betterCall.APIError("UNAUTHORIZED", {
          message: ERROR_CODES.UNAUTHORIZED_SESSION
        });
      }
      if (session$1 && ctx.body.userId && session$1?.user.id !== ctx.body.userId) {
        throw new betterCall.APIError("UNAUTHORIZED", {
          message: ERROR_CODES.UNAUTHORIZED_SESSION
        });
      }
...
async function validateApiKey({
  ...
  if (permissions) {
    ...
    const r = access.role(apiKeyPermissions);
...
const oneTimeToken = (options) => {
  ...
  return {
    ...
    endpoints: {
       ...
       */
      generateOneTimeToken: middleware.createAuthEndpoint(
       ...
       */
      verifyOneTimeToken: middleware.createAuthEndpoint(
