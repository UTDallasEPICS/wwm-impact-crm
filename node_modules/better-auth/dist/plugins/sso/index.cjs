...
const sso = (options) => {
  return {
    ...
    endpoints: {
       ...
       */
      createOIDCProvider: middleware.createAuthEndpoint(
        "/sso/register",
        {
          method: "POST",
       ...
       */
      signInSSO: middleware.createAuthEndpoint(
        "/sign-in/sso",
        {
          method: "POST",
          body: z__namespace.object({
        ...
        async (ctx) => {
          ...
          const state = await toAuthEndpoints.generateState(ctx);
      ...
      callbackSSO: middleware.createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          const stateData = await toAuthEndpoints.parseState(ctx);
          if (!stateData) {
          ...
          const linked = await toAuthEndpoints.handleOAuthUserInfo(ctx, {
            userInfo: {
            ...
            account: {
              ...
              accessToken: await toAuthEndpoints.setTokenUtil(
                tokenResponse.accessToken,
                ctx.context
              ),
              refreshToken: await toAuthEndpoints.setTokenUtil(
