...
const cookies_index = require('../../shared/better-auth.DXPBskCs.cjs');
const error = require('@better-auth/core/error');
const oauth2 = require('@better-auth/core/oauth2');
const toAuthEndpoints = require('../../shared/better-auth.BxKoxSCD.cjs');
const session = require('../../shared/better-auth.DHYnDXUq.cjs');
...
const genericOAuth = (options) => {
  ...
  return {
    ...
    endpoints: {
       ...
       */
      signInWithOAuth2: middleware.createAuthEndpoint(
        "/sign-in/oauth2",
        {
          method: "POST",
        ...
        async (ctx) => {
          ...
          const { state, codeVerifier } = await toAuthEndpoints.generateState(ctx);
      ...
      oAuth2Callback: middleware.createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          let tokens = void 0;
          const parsedState = await toAuthEndpoints.parseState(ctx);
          const {
            callbackURL,
            codeVerifier,
            errorURL,
          ...
          try {
            ...
            tokens = await oauth2.validateAuthorizationCode({
          ...
          const result = await toAuthEndpoints.handleOAuthUserInfo(ctx, {
            userInfo,
            account: {
              providerId: provider.providerId,
              accountId: userInfo.id,
       ...
       */
      oAuth2LinkAccount: middleware.createAuthEndpoint(
        "/oauth2/link",
        {
          method: "POST",
          body: z__namespace.object({
        ...
        async (c) => {
          ...
          if (!provider) {
            throw new betterCall.APIError("NOT_FOUND", {
              message: error.BASE_ERROR_CODES.PROVIDER_NOT_FOUND
            });
          }
          const {
            providerId,
          ...
          const state = await toAuthEndpoints.generateState(c, {
