...
export default defineEventHandler(async (event) => {
  try {
    ...
    const exists = await prisma.pledgePayment.findUnique({ where: { id } })
    if (!exists) {
      event.node.res.statusCode = 404
      return { error: 'PledgePayment not found' }
    }

    const updated = await prisma.pledgePayment.update({
      where: { id },
      data: {
        pledgeId: body?.pledgeId ?? undefined,
    ...
    return { error: 'Failed to update pledge payment', details: err }
  }
})
